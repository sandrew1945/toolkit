 ...
 * @version    :
 */
public class POI07ExcelParseImpl implements ExcelParser
{
	 ...
	 * @return
	 */
	public static POI07ExcelParseImpl newInstance(RowRule rRule, int aTitleRows, Integer aTotalCol)
	{
		POI07ExcelParseImpl parser = new POI07ExcelParseImpl();
		parser.rRule = rRule;
	...
	public ExcelResult parse(InputStream fis)
	{
		try
		{
			...
			{
				XSSFSheet sheet = wb.getSheetAt(curSheet);
				...
				for (int curRow = titleRows; curRow < rowNum; curRow++)
				{
					...
					{
						//保存在LIST中
						XSSFCell curCell = sheet.getRow(curRow).getCell(colNum);
